...
async fn create(context: &Context, arguments: Value) -> Result<Value, String> {
    ...
    let row = tx.query_one(
        "INSERT INTO scheduled_tasks.tasks (user_id, team_id, project_id, name, prompt, cron, timezone, next_run_at) VALUES (current_app_user(), $1, $2, $3, $4, $5, $6, $7) RETURNING id, name, prompt, cron, timezone, enabled, next_run_at",
        &[&context.team_id, &context.project_id, &name, &prompt, &cron, &timezone, &next_run_at],
