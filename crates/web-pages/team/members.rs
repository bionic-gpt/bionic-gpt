...
pub fn page(
    ...
    let team_public_id = db::team_public_id::encode(team.id).unwrap_or_else(|| team.id.to_string());
    ...
    let page = rsx! {
        AdminLayout {
            ...
            header: rsx!(
                Breadcrumb {
                    items: vec![
                        BreadcrumbItem {
                            text: "Teams".into(),
                            href: Some(crate::routes::teams::Switch{team_id: team_public_id.clone()}.to_string())
                        },
                        BreadcrumbItem {
            ...
            div {
                        ...
                        CardBody {
                            ...
                            if user.first_name.is_none() {
                                p {
                                    "Please set your "
                                    a {
                                        href: crate::routes::profile::Profile{team_id: team_public_id.clone()}.to_string(),
                                        "name"
                                    }
                                }
                            }
                ...
                for member in members {
                    ConfirmModal {
                        action: crate::routes::team::Delete{team_id: team_public_id.clone()}.to_string(),
                ...
                for invite in invites {
                    ConfirmModal {
                        action: crate::routes::team::DeleteInvite{team_id: team_public_id.clone()}.to_string(),
                ...
                super::invitation_form::InvitationForm {
                    submit_action: crate::routes::team::CreateInvite{team_id:team_public_id.clone()}.to_string()
                }

                // Form to set he org name
                super::team_name_form::TeamNameForm {
                    submit_action: crate::routes::team::SetName{team_id:team_public_id.clone()}.to_string()
                }
            }
        }
    };
