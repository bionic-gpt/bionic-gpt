...
clarifying questions, iterative drafting, document generation, and persistent
artifact generation.

The document co-authoring skill and document-generation capability are system
capabilities. They should already be available after the database migrations
have run.
...
- Does it create an editable source document rather than only returning chat text?
...
Then create an editable document and a compiled PDF. Keep the document
suitable for review by engineering leadership.
```
...
Inspect both the editable source document and the PDF. Confirm that the second request
