...
pub fn execute_tool_call(tool_call: &ToolCall) -> Result<ChatCompletionMessage, String> {
    // Get all available tools
    let tools = function_tools::get_tools();
    if let Some(tools) = tools {
        execute_tool_call_with_tools(&tools, tool_call)
    } else {
        Err("Unable to call tool".to_string())
    }
}
...
mod tests {
    ...
    fn test_execute_tool_call_weather() {
        ...
        assert_eq!(result.role, ChatCompletionMessageRole::Tool);
